...
function onBlockBuddyBtClick()
{
	...
	sfs.send(new SFS2X.Requests.BuddyList.BlockBuddyRequest(selectedBuddy.title, !isBlocked));
}
...
{
	var selectedBuddy = $("#buddyList").jqxListBox("getSelectedItem");
	sfs.send(new SFS2X.Requests.BuddyList.RemoveBuddyRequest(selectedBuddy.title));
...
function onAddBuddyBtClick()
{
	if ($("#buddyNameIn").val() != "")
	{
		sfs.send(new SFS2X.Requests.BuddyList.AddBuddyRequest($("#buddyNameIn").val()));
		$("#buddyNameIn").val("");
	}
}
...
function onIsOnlineChange(event)
{
	...
	if (sfs.buddyManager.getMyOnlineState() != goOnline)
		sfs.send(new SFS2X.Requests.BuddyList.GoOnlineRequest(goOnline));
...
{
	var nickBV = new SFS2X.Entities.Variables.SFSBuddyVariable(SFS2X.Entities.Variables.ReservedBuddyVariables.BV_NICKNAME, $("#nicknameIn").val());
	sfs.send(new SFS2X.Requests.BuddyList.SetBuddyVariablesRequest([nickBV]));
}
...
function onSetAgeBtClick(event)
{
	...
	var ageBV = new SFS2X.Entities.Variables.SFSBuddyVariable(BUDDYVAR_AGE, age);
	sfs.send(new SFS2X.Requests.BuddyList.SetBuddyVariablesRequest([ageBV]));
}
...
function onSetMoodBtClick(event)
{
	...
	var moodBV = new SFS2X.Entities.Variables.SFSBuddyVariable(BUDDYVAR_MOOD, mood);
	sfs.send(new SFS2X.Requests.BuddyList.SetBuddyVariablesRequest([moodBV]));
}
...
function onStateChange(event)
{
	...
	var stateBV = new SFS2X.Entities.Variables.SFSBuddyVariable(SFS2X.Entities.Variables.ReservedBuddyVariables.BV_STATE, state);
	sfs.send(new SFS2X.Requests.BuddyList.SetBuddyVariablesRequest([stateBV]));
}
...
function onSendBtClick(event)
{
	...
	if ($(msgInputId).val() != "")
	{
		...
		var params = {};
		params.recipient = buddy.name;

		var isSent = sfs.send(new SFS2X.Requests.BuddyList.BuddyMessageRequest($(msgInputId).val(), buddy, params));
...
function onBuddyMessage(event)
{
	...
	if (isItMe)
	{
		var buddyName = event.data.recipient;
...
{
	if (buddy.getNickName() != null && buddy.getNickName() != "")
