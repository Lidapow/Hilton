(function(){
	...
	this.onMinimapClick = function(e)
	{
		...
		// Join MMORoom
		sfs.send(new SFS2X.Requests.System.JoinRoomRequest(MMO_ROOM_NAME));
	}
	...
	this.onSendPublicMessageBtClick = function(e)
	{
		if ($("#publicMsgIn").val() != "")
		{
			sfs.send( new SFS2X.Requests.System.PublicMessageRequest($("#publicMsgIn").val()) );
			$("#publicMsgIn").val("");
		}
	}
	...
	{
		var userVars = [];
		userVars.push(new SFS2X.Entities.Variables.SFSUserVariable(USERVAR_X, px));
		userVars.push(new SFS2X.Entities.Variables.SFSUserVariable(USERVAR_Y, py));

		if (dir != null)
			userVars.push(new SFS2X.Entities.Variables.SFSUserVariable(USERVAR_DIR, dir));

		sfs.send( new SFS2X.Requests.System.SetUserVariablesRequest(userVars) );
	}
	...
	function updateServerPosition()
	{
		...
		var pos = new SFS2X.Entities.Data.Vec3D(myAvatar.lastUpdateX, myAvatar.lastUpdateY, 0);
		sfs.send( new SFS2X.Requests.MMO.SetUserPositionRequest(pos) );
	}
